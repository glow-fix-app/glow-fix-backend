...
var MetricsInterceptor = function () {
    ...
    var MetricsInterceptor = _classThis = /** @class */ (function () {
        function MetricsInterceptor_1(metricsService) {
            this.metricsService = metricsService;
        }
        MetricsInterceptor_1.prototype.intercept = function (context, next) {
            var _this = this;
            var _a;
            var request = context.switchToHttp().getRequest();
            var method = request.method;
            // Normalize route for metrics (replace UUIDs with :id)
            var route = this.normalizeRoute(((_a = request.route) === null || _a === void 0 ? void 0 : _a.path) || request.url);
            var endTimer = this.metricsService.httpRequestDuration.startTimer({
                method: method,
                route: route,
            });
            return next.handle().pipe((0, operators_1.tap)({
                next: function () {
                    ...
                    _this.metricsService.httpRequestTotal.inc({
                        method: method,
                        route: route,
                ...
                error: function (error) {
                    ...
                    _this.metricsService.httpErrorTotal.inc({
                        method: method,
                        route: route,
                        status_code: statusCode.toString(),
        ...
        MetricsInterceptor_1.prototype.normalizeRoute = function (path) {
            return path
