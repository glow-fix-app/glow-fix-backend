...
//     // Build the base query using Prisma.sql to avoid SQL injection and type issues
//     let sql = Prisma.sql`
//       SELECT 
//         b.id,
//         b.business_name,
...
//     if (userLat && userLng && filters?.max_distance) {
//       // This needs to be handled differently - we'll filter after query
//     }
...
//     // Apply max distance filter (since it's a spatial calculation in HAVING)
//     let filteredProviders = providers;
//     if (userLat && userLng && filters?.max_distance) {
//       filteredProviders = providers.filter(
//         (p) => p.distance_km !== null && p.distance_km <= filters.max_distance!,
...
//     // Get total count
//     let total = filteredProviders.length;

//     // Get business locations for providers without coordinates
//     const businessIds = filteredProviders.map((p) => p.id);
//     const locationsMap = await this.getBusinessLocationsBatch(businessIds);
...
//     // Apply sorting for distance-based sorts (done in SQL, but ensure consistency)
//     if (sort_by === ProviderSortBy.NEAREST && !userLat && !userLng) {
//       formattedProviders.sort((a, b) => b.average_rating - a.average_rating);
//     }
...
//     const paginatedData = formattedProviders.slice(0, take);
